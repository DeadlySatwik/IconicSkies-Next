...
export function FavoriteLocationsGrid({
  ...
  const summary = favorites.length === 0
    ? "Save cities like Home, Hostel, or Work"
    : `${favorites.length} saved place${favorites.length === 1 ? "" : "s"}`;

  if (favorites.length === 0) {
    return (
      <CollapsibleSection
        action={<Link className="inline-flex min-h-10 items-center gap-2 rounded-full bg-cloud px-4 py-2 text-sm font-semibold text-skyInk" href="/">Search weather</Link>}
        defaultOpen={defaultOpen}
        subtitle="Your favorite skies appear here with a live preview so you can check the weather at a glance."
        summary={summary}
        title="My Places"
        variant="dark"
      >
        <div className="p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="max-w-2xl">
              <h3 className="text-2xl font-semibold text-cloud">Save cities like Home, Hostel, or Work</h3>
              <p className="mt-2 text-sm leading-6 text-cloud/82">
                Your favorite skies appear here with a live preview so you can check the weather at a glance.
              </p>
            </div>
          </div>
          <p className="mt-4 text-sm text-cloud/76">Up to 6 favorites, each with a calm live preview.</p>
        </div>
      </CollapsibleSection>
    );
  }
  ...
  return (
    <CollapsibleSection
      action={<Link className="hidden items-center gap-2 text-sm font-semibold text-cloud/86 hover:text-cloud sm:inline-flex" href="/">Search another city <ArrowRight aria-hidden className="size-4" /></Link>}
      defaultOpen={defaultOpen}
      subtitle="Live weather previews for your saved places."
      summary={summary}
      title="Favorite skies"
      variant="dark"
    >
      <div className="p-5">
