...
export function listFallbackMoments(userId: string) {
  ...
  return moments.map((moment) => {
    ...
    return {
      ...
      weatherId: snapshot?.weatherId ?? null,
      cloudiness: snapshot?.cloudiness ?? null,
      timezoneOffset: snapshot?.timezoneOffset ?? null,
      sunrise: snapshot?.sunrise ?? null,
      sunset: snapshot?.sunset ?? null,
